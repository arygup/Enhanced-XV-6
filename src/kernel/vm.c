...
extern int nump_using[(PGROUNDUP(PHYSTOP)/PGSIZE)];  // COW
extern struct spinlock nump_using_lock;  // COW
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    // COW
    // checking if the page is COW or not. If it is, then we need to copy the page to a new page.
